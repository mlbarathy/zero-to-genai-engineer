"""
PRESERVED COPY — LangChain `create_agent` research Runtime (NOT a hand-built StateGraph).

This is the working create_agent + streaming SSE demo:
  Memory + Gateway MCP + Identity + RAG (search_docs).

Sibling with explicit LangGraph nodes/edges:
  langgraph_research_graph_agentcore.py

Configure & launch (this create_agent variant):
  agentcore configure -e research_create_agent_agentcore.py -n research_create_agent
  agentcore launch -a research_create_agent ...
"""
...
def _normalize_browse_url(url: str) -> str | None:
    """Require http(s) so the cloud browser is not pointed at junk schemes."""
    raw = (url or "").strip()
    if not raw:
        return None
    if "://" not in raw:
...
async def _browse_url_async(url: str) -> str:
    """AgentCore managed Chromium → Playwright CDP → page text + live-view URL."""
    ...
    live_url = ""
    try:
        # 👇 Cloud Chromium sandbox (not your laptop browser)
        host = urlparse(url).netloc.replace(".", "-")[:24] or "page"
        session_id = client.start(name=f"research-{host}"[:48])
        try:
            live_url = client.generate_live_view_url(expires=300)
        except Exception as exc:  # noqa: BLE001
            live_url = f"(live view unavailable: {exc})"

        ws_url, ws_headers = client.generate_ws_headers()
        async with async_playwright() as pw:
            # Connect over CDP into the AgentCore browser session
            ...
            # Prefer visible text; strip scripts/styles in-page
        ...
        return (
            ...
            f"(Open live_view_url within ~5 min to watch the cloud browser.)\n\n"
            f"{body or '(empty page body)'}"
        )
    except Exception as exc:  # noqa: BLE001
        return (
            f"Browser tool error: {exc}\n"
            f"url={url}\n"
            f"session_id={session_id or 'n/a'}\n"
            f"live_view_url={live_url or 'n/a'}\n"
            "Ensure Runtime IAM allows StartBrowserSession / ConnectBrowserAutomationStream."
        )
    finally:
        try:
            client.stop()
...
def browse_url(url: str) -> str:
    """Open a live webpage in AgentCore Browser (managed cloud Chromium) and return its text.

    Use for reading a specific URL (docs page, blog, product page, docs.aws.amazon.com).
    Prefer web_search for open-ended research; prefer search_docs for our RAG corpus.

    Args:
        url: Full http(s) URL to open (e.g. https://docs.aws.amazon.com/...).
    """
    normalized = _normalize_browse_url(url)
    if not normalized:
        return "Provide a valid http(s) URL, e.g. https://example.com/page"
    try:
        return asyncio.run(_browse_url_async(normalized))
    except RuntimeError:
        # Nested event loop (rare) — fall back to a fresh loop
        loop = asyncio.new_event_loop()
        try:
            return loop.run_until_complete(_browse_url_async(normalized))
        finally:
            loop.close()
...
TOOL_LABELS = {
    ...
    "browse_url": "Browsing page in AgentCore Browser",
...
def _tools_available_list(stack: dict) -> list[dict]:
    tools_available = [
        ...
        {
            "name": "browse_url",
            "label": TOOL_LABELS["browse_url"],
            "source": "agentcore_browser",
        },
    ]
...
SYSTEM_PROMPT = """You are a helpful research assistant with local tools, RAG docs, AgentCore Browser, and optional Gateway MCP tools.

When to use tools (only if needed):
1. AgentCore / Lauki FAQ knowledge → search_docs (RAG).
2. Public facts / news / open-ended discovery → web_search.
3. Read a specific live webpage (given URL or after search finds a URL) → browse_url.
4. Current UTC date/time → get_current_datetime.
5. Arithmetic → calculator (do not guess).
6. Weather or city local time → Gateway MCP get_weather / get_time when available.
...
- Do not call browse_url for greetings or math.

Rules:
- Prefer zero tools when a direct answer is enough.
- Prefer web_search first for "what is X"; use browse_url when you need the actual page content.
- If browse_url returns a live_view_url, mention it so the user can watch the cloud browser.
